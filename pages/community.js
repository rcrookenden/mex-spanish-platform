...
export type Option = {
  ...
  feedback: string;
  retry?: boolean;
  translation?: string;   // kept for compatibility; rendered under the FEEDBACK caption (not under buttons)
  feedbackTr?: string;    // shows under the feedback caption in italics
};
...
export type SuperChunkBlock = {
  kind: "superChunk";
  chunk: string;
  prompt1: string;               // may contain inline HTML
  options1: Option[];
  prompt2?: string;              // optional second task (translation)
  answer2?: string;
  xpExact?: number;
  xpFuzzy?: number;
  xpWrong?: number;
...
/* NEW: Rearrange sentence block */
export type RearrangeBlock = {
  kind: "rearrange";
  prompt: string;
  answer: string[];          // array of tokens in the correct order
  translation?: string;      // English gloss for feedback caption
  xpCorrect: number;         // e.g., 20
  xpWrong: number;           // e.g., -5
...
export const Act1: Mission = {
  ...
  scenes: [
    ...
    /* ───── Scene 5 – SUPER CHUNK – Se me hace que… ───────── */
    {
      id: "scene-5",
    ...
    /* ───── Scene 6 – Rearrange Sentence (NEW) ───────── */
    {
      id: "scene-6",
      title: "🧩 REARRANGE — Se me hace que…",
      blocks: [
        {
          kind: "rearrange",
          prompt: "Rearrange the words to form the correct sentence:",
          answer: ["se", "me", "hace", "que", "no", "va", "a", "venir"],
          translation: "I get the feeling he/she is not going to come.",
          xpCorrect: 20,
          xpWrong: -5,
          feedbackCorrect: "¡Exacto! 🚀 Se me hace que no va a venir.",
          feedbackWrong: "Casi, pero no. The correct sentence is: “Se me hace que no va a venir.”"
        }
      ]
    }
  ]
    ...
    >
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-10 py-6">
        <div className="rounded-3xl bg-[#0b1518]/80 backdrop-blur-xl border border-slate-800/80 shadow-[0_25px_60px_-25px_rgba(0,0,0,0.8)] p-6 sm:p-8">
          ...
          {/* NEW: Rearrange Block */}
          {current?.kind === "rearrange" && (
            <RearrangeBlockView
              block={current as RearrangeBlock}
              onAward={(d) => awardXP(d)}
              sfx={sfx}
              onNext={() => next()}
              onBack={back}
...
function RearrangeBlockView({
  ...
  block: RearrangeBlock;
  onAward: (delta: number) => void;
  sfx: ReturnType<typeof useAnswerSfxFiles>;
  onNext: () => void;
  onBack: () => void;
}) {
  const gold = block.answer;
  const [pool] = React.useState<string[]>(() => shuffle(gold));
  const [picked, setPicked] = React.useState<string[]>([]);
  const [usedIdx, setUsedIdx] = React.useState<Set<number>>(new Set());
  const [result, setResult] = React.useState<null | { ok: boolean; msg: string; xp: number }>(null);

  function togglePick(i: number) {
    if (result) return;
    if (usedIdx.has(i)) return;
    setPicked((p) => [...p, pool[i]]);
    setUsedIdx((s) => new Set(s).add(i));
  }
  function undo() {
    if (result) return;
    setPicked((p) => {
      ...
      const idx = pool.findIndex((t, ii) => t === last && usedIdx.has(ii));
      setUsedIdx((s) => {
        const ss = new Set([...s]);
        // free only one matching token index
        for (let j = pool.length - 1; j >= 0; j--) {
          if (pool[j] === last && ss.has(j)) { ss.delete(j); break; }
        }
        return ss;
      });
      return p.slice(0, -1);
  ...
  function check() {
    if (result) return;
    const ok = picked.join(" ") === gold.join(" ");
    const xp = ok ? block.xpCorrect : block.xpWrong;
    onAward(xp);
    if (ok) sfx.correct(); else sfx.wrong();
    const msg = ok
      ? (block.feedbackCorrect || "¡Correcto!")
      : (block.feedbackWrong || "Not quite.");
    setResult({ ok, msg, xp });
  }

  const canCheck = picked.length === gold.length;

  return (
    <div className="space-y-6">
      <div className="relative overflow-hidden rounded-3xl border-2 border-amber-400/60 bg-gradient-to-r from-amber-300/20 via-rose-300/20 to-emerald-300/20 p-6 text-center">
        <div className="text-2xl sm:text-3xl font-black text-amber-300">SUPER CHUNK ALERT 🚨</div>
        <div className="mt-2 text-emerald-200/90 text-2xl font-extrabold">Se me hace que…</div>
      </div>

      <div className="rounded-2xl border border-emerald-700/40 bg-[#0f1f23]/70 p-5 shadow-[inset_0_0_0_1px_rgba(16,185,129,0.2)]">
        <div className="font-semibold text-emerald-200 mb-3">{block.prompt}</div>
            ...
            return (
              <button
                ...
                className={`px-3 py-1.5 rounded-xl border text-sm font-bold transition
                  ${used ? "opacity-40 cursor-not-allowed" : "cursor-pointer hover:bg-slate-800"}
                  bg-slate-900/60 text-slate-100 border-slate-600/60`}
              >
                {tok}
              </button>
            );
        ...
        {/* Output */}
        <div className="mt-4 rounded-xl bg-slate-900/50 border border-slate-700 p-3 min-h-[52px]">
          <span className="text-slate-100 font-extrabold">{picked.join(" ")}</span>
        </div>
        ...
        <div className="mt-4 flex items-center gap-2">
          <button
            onClick={undo}
            disabled={picked.length === 0 || !!result}
            className={`px-3 py-2 rounded-xl font-extrabold border transition
              ${picked.length === 0 || result ? "bg-slate-700/50 cursor-not-allowed" : "bg-slate-800/70 hover:bg-slate-700/70 cursor-pointer"} border-slate-600/60 text-slate-100`}
          >
            Undo
          </button>
          <button
            onClick={reset}
            disabled={(picked.length === 0 && usedIdx.size === 0) && !result}
            className="px-3 py-2 rounded-xl font-extrabold border bg-slate-800/70 hover:bg-slate-700/70 border-slate-600/60 text-slate-100 transition cursor-pointer"
          >
            Reset
          </button>
          ...
          <button
            ...
            disabled={!canCheck}
            className={`min-w-36 text-center text-lg px-6 py-3 rounded-2xl font-extrabold text-slate-900 transition
              ${!canCheck ? "bg-slate-600/60 cursor-not-allowed" : "bg-emerald-300 hover:bg-emerald-200 cursor-pointer"} shadow-[0_10px_30px_-10px_rgba(110,231,183,0.6)]`}
          >
            Check
          </button>
        </div>
        ...
        {result && (
          <div
            className={`mt-4 rounded-2xl px-5 py-4 font-semibold leading-snug border-2
              ${result.ok ? "bg-emerald-100 text-emerald-900 border-emerald-400" : "bg-rose-100 text-rose-900 border-rose-400"}`}
          >
            {result.msg} <span className="font-black">{result.xp >= 0 ? `+${result.xp}` : result.xp} XP</span>
            {block.translation && (
              <em className="mt-1 block italic text-[0.975rem] leading-snug text-emerald-900/80">
        ...
        <div className="mt-4">
          <NavRow onBack={onBack} onNext={onNext} disabledNext={!result?.ok} />
        </div>
      </div>
    </div>
  );
