...
export type DialogueTurn = { speaker: "Receptionist" | "You" | "Waiter" | "Narration" | string; text: string };
...
export const Act2_Scene1: Mission = {
  id: "act-2-hotel-breakfast",
  title: "ACT 2 – Breakfast Before the Storm",
  preload: [
    {
      kind: "narrative",
      text:
        "🎬 STORY BEAT – Morning\n" +
        "You wake to sunlight slipping through the thin curtains, your mind still foggy from yesterday’s trip… and that note.\n" +
        "The hallway is quiet as you take the creaky elevator down. The smell of fresh coffee and tortillas drifts in from the small hotel café.\n" +
        "You pick a table by the window."
    },
    {
      kind: "card",
      title: "🎒 CHUNK PRELOAD (Matching Exercise)",
      body:
        "¡Y ya! → And (do it) right now!!\n" +
        "Voy a querer… → I’ll have… (casual, very common in Mexico)\n" +
        "Enseguida se lo traigo, señor. → Coming right up, sir.\n" +
        "¡Deja de chingar la madre! → Stop fucking pestering me!\n" +
        "Mucho gusto. → Nice to meet you.\n" +
        "Te disparo + comida/bebida → I’ll buy you + food/drink\n" +
        "La cuenta, por favor. → The bill, please.\n" +
        "Está bueno. → It’s good.\n" +
        "¿Cómo que no llegaste? → What do you mean you didn’t arrive?"
    }
  ],
  scenes: [
    {
      ...
      blocks: [
        {
          kind: "choiceLead",
          lead:
            "A waiter appears with a warm smile.\n\n" +
            "Waiter: ¿Le traigo la carta?",
          ...
          options: [
            {
              ...
              correct: true,
              xp: 10,
              feedback: "Yep, that’s all that’s needed here. Simples."
            },
            {
              id: "a2s1-o2",
              text: "¿Me puede traer la carta?",
              translation: "Can you bring me the menu?",
              correct: false,
              xp: -3,
              feedback:
                "Confusion brushes the waiter’s face, but he composes himself quickly. This phrasing is redundant given the waiter’s question.",
              retry: true
            },
            {
              id: "a2s1-o3",
              text: "Tráigame la carta y ya.",
              translation: "Bring me the menu already.",
              correct: false,
              xp: -3,
              feedback:
                "The poor waiter doesn’t know what to say! WAY too rude!",
              retry: true
            }
          ]
        },
        {
          kind: "choiceLead",
          lead:
            "The waiter comes back with the menu.\n\nSystem Prompt: How do you order like a NATIVE?",
          prompt: "How do you order breakfast?",
          options: [
            {
              id: "a2s1-o4",
              text: "Me gustaría unos chilaquiles verdes, por favor.",
              translation: "I would like some green chilaquiles, please.",
              correct: true,
              xp: 5,
              feedback:
                "Technically correct, but not what Mexicans usually say. Still polite and fine."
            },
            {
              id: "a2s1-o5",
              text: "Voy a querer unos huevos rancheros y un jugo de naranja, por favor.",
              translation: "I’ll have ranch-style eggs and an orange juice, please.",
              correct: true,
              xp: 10,
              feedback: "Yep, you nailed it! Very natural."
            },
            {
              id: "a2s1-o6",
              text: "Puedo tener unos huevos rancheros y un jugo de naranja, por favor.",
              translation: "Can I have ranch-style eggs and an orange juice, please?",
              correct: false,
              xp: -3,
              feedback:
                "Too literal! Thankfully, the waiter still understands.",
              retry: true
            }
          ]
        },
        {
          kind: "dialogueWithChoice",
          intro: "Waiter: Enseguida se lo traigo, señor. ¿Gusta algo más?",
          dialogue: [
            { speaker: "Waiter", text: "Enseguida se lo traigo, señor. ¿Gusta algo más?" }
          ],
          prompt: "How do you respond?",
          options: [
            {
              id: "a2s1-o7",
              text: "No por el momento, gracias.",
              translation: "Not for the moment, thanks.",
              correct: true,
              xp: 10,
              feedback: "Polite and simple. Good job. The waiter smiles."
            },
            {
              id: "a2s1-o8",
              text: "No.",
              ...
              correct: false,
              xp: -3,
              feedback: "Sounds abrupt and unfriendly. Manners maketh man.",
              retry: true
            },
            {
              id: "a2s1-o9",
              text: "Ya deja de chingar la madre, ¿no?",
              translation: "Stop fucking bothering me, will you?",
              correct: false,
              xp: -10,
              feedback: "The waiter flinches. WAY too rude! I can’t believe you said that!",
              retry: true
            }
          ]
        }
...
export default function Page() {
  return (
    <div className="relative">
      <MissionRunner mission={Act2_Scene1} />

      {/* Sticky NEXT ACT button (adjust href to your next route) */}
      <div className="fixed right-4 bottom-4 z-[1100]">
        <Link
          href="/missions/inheritance/act-2/scene2"
          className="px-5 py-3 rounded-2xl font-extrabold text-slate-900 bg-amber-300 hover:bg-amber-200 shadow-[0_10px_30px_-10px_rgba(252,211,77,0.6)]"
        >
          NEXT ACT →
        </Link>
      </div>
...
export function MissionRunner({ mission }: { mission: Mission }) {
  ...
  const goNext = React.useCallback(() => {
    if (isFinalStep) router.push("/missions/inheritance/act-2/scene2");
    else next();
  ...
  // Morning hero image (swap as needed)
  const heroImage =
    currentSceneIdx >= 0 ? "/images/cafe-desayuno.png" : "/images/hotel-san-lazaro.png";

  return (
    <div
      className="relative min-h-screen bg-[#12261f] text-slate-100 overflow-x-hidden"
    ...
    >
      ...
      {/* Backgrounds — lighter morning vibe */}
      <div className="absolute inset-0 -z-10 bg-[url('/talavera.svg')] bg-center bg-cover opacity-[0.06]" />
      <div
        className="absolute inset-0 -z-10 pointer-events-none"
        style={{
          background:
            "radial-gradient(1200px 600px at 50% 10%, rgba(255,255,255,0.10), transparent 55%), " +
            "radial-gradient(900px 500px at 15% 0%, rgba(253,230,138,0.22), transparent 55%), " +
            "linear-gradient(180deg, rgba(10,40,32,0.35), rgba(0,0,0,0.06))"
        }}
      />

      {/* Top glow bar */}
      <div className="pointer-events-none absolute left-0 right-0 top-0 h-1 bg-gradient-to-r from-amber-200 via-emerald-200 to-rose-300/90 blur-[1px]" />

      {/* HERO */}
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-10 pt-10 pb-6 text-center">
        <div className="inline-block relative">
          <div className="absolute -inset-1 rounded-3xl bg-amber-400/15 blur-xl" />
          <div className="relative px-6 sm:px-10 py-5 sm:py-6 bg-[#142b24]/80 backdrop-blur-md border-2 border-amber-400/50 rounded-3xl shadow-[0_10px_40px_-20px_rgba(251,191,36,0.35)]">
            <h1 className="text-3xl sm:text-5xl text-amber-200 drop-shadow">🔑 The Inheritance Mystery</h1>
            <p className="mt-2 text-2xl sm:text-3xl text-emerald-200/90">{currentSceneTitle}</p>
          </div>
        </div>
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-10">
        <div className="rounded-2xl bg-[#142820]/65 backdrop-blur border border-emerald-600/30 p-4 sm:p-5 shadow-[0_10px_40px_-20px_rgba(0,0,0,0.6)]">
          <div className="flex flex-wrap items-center justify-between gap-4">
            <div className="text-sm sm:text-base font-extrabold text-amber-200">
              Step {step + 1}/{totalSteps}
            </div>
            <div className="text-sm sm:text-base font-extrabold text-emerald-200">XP: {xp}</div>
          </div>
          <div className="mt-3 relative h-3 rounded-full bg-slate-700/60 overflow-hidden border border-slate-600/70">
            <div
              className="h-full bg-gradient-to-r from-amber-300 via-emerald-300 to-rose-400 transition-all duration-500"
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-10 py-6">
        <div className="rounded-3xl bg-[#13271f]/78 backdrop-blur-xl border border-slate-700/70 shadow-[0_25px_60px_-25px_rgba(0,0,0,0.75)] p-6 sm:p-8">
      ...
      {showNext && (
        <button
          className={`min-w-36 text-center text-lg px-6 py-3 rounded-2xl font-extrabold text-slate-900 transition
            ${disabledNext ? "bg-slate-600/60 cursor-not-allowed" : "bg-amber-300 hover:bg-amber-200 cursor-pointer"} shadow-[0_10px_30px_-10px_rgba(252,211,77,0.6)]`}
...
function DialogueWithChoiceBlock({
  ...
  // 🔊 Dialogue pop for Waiter/You lines
  React.useEffect(() => {
    if (!delayReady || !line) return;
    if (line.speaker === "Waiter" || line.speaker === "You") {
      sfx.pop();
    }
  ...
  const helper =
    !delayReady ? "Please wait…"
    : !doneTyping ? "Click to skip typing"
    : !atLastLine ? "Click to continue →"
    : !showQuestion ? "Reveal question ↓"
    : `Answer to unlock ${nextLabel}`;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className={`relative transition-opacity ${delayReady ? "opacity-100" : "opacity-70"}`}>
        <div
          className={`rounded-3xl border-2 border-emerald-600/30 bg-[#143128]/80 backdrop-blur px-5 sm:px-7 py-5 sm:py-6 shadow-[0_10px_40px_-20px_rgba(16,185,129,0.35)] ${delayReady ? "cursor-pointer" : "cursor-not-allowed"}`}
        ...
        >
          <div className="mb-3">
            <span className="inline-block px-3 py-1 rounded-full text-xs font-black tracking-wide
              bg-amber-300 text-emerald-950 shadow">{line?.speaker ?? "…"}</span>
          </div>

          <div className="min-h-[64px] text-[1.075rem] leading-relaxed text-slate-100">
            {delayReady ? fullText.slice(0, charIdx) : "…"}
            {delayReady && !doneTyping && <span className="ml-0.5 text-amber-300/90 animate-pulse">▍</span>}
          </div>
...
/* ─────────────── SUPER CHUNK VIEW (unchanged from Act 1 style) ─────────────── */
    ...
    <div className="relative space-y-6">
      {showUnlock && (
        <div className="pointer-events-none fixed inset-0 z-[950] flex items-start justify-center">
          <div className="mt-20 px-4 py-2 rounded-full font-extrabold text-emerald-950 bg-amber-300/95 shadow-lg">
      ...
      {/* MCQ */}
      <div className="rounded-2xl border border-emerald-700/40 bg-[#15322a]/70 p-5 shadow-[inset_0_0_0_1px_rgba(16,185,129,0.16)]">
        <div
        ...
        <div className="rounded-2xl border border-emerald-700/40 bg-[#15322a]/70 p-5 shadow-[inset_0_0_0_1px_rgba(16,185,129,0.16)]">
          ...
          <input
            ...
            className={`w-full rounded-xl px-4 py-3 bg-slate-900/50 border outline-none transition
              ${!mcqDone ? "opacity-60 cursor-not-allowed" : "cursor-text border-slate-700 focus:border-amber-400"}
              text-slate-100`}
          />
          ...
          <div className="mt-4 flex items-center justify-between">
            ...
            {!result2 ? (
              <button
                ...
                className={`min-w-36 text-center text-lg px-6 py-3 rounded-2xl font-extrabold text-slate-900 transition
                  ${(!mcqDone || input.trim().length === 0) ? "bg-slate-600/60 cursor-not-allowed" : "bg-amber-300 hover:bg-amber-200 cursor-pointer"} shadow-[0_10px_30px_-10px_rgba(252,211,77,0.6)]`}
              >
                Check answer
              </button>
            ) : (
              <button
                onClick={onNext}
                className="min-w-36 text-center text-lg px-6 py-3 rounded-2xl font-extrabold text-slate-900 bg-amber-300 hover:bg-amber-200 shadow-[0_10px_30px_-10px_rgba(252,211,77,0.6)] transition cursor-pointer"
              >
                {nextLabel}
              </button>
            )}
...
function MatchCard({
  ...
  return (
    <div className="space-y-6">
      <div className="rounded-2xl border border-emerald-700/50 bg-[#15322a]/70 p-4 shadow-[inset_0_0_0_1px_rgba(16,185,129,0.16)]">
...
function useDialogueBlip() {
  ...
  const play = React.useCallback((speaker?: string) => {
    ...
    const base = speaker === "Waiter" ? 1.06 : speaker === "You" ? 0.96 : 1.0;
...
function ToastBubble({
  ...
  const sign = positive ? "+" : "";
  const bg = positive ? "rgba(252,211,77,0.95)" : "rgba(244,63,94,0.95)";
...
function TypeLine({
  ...
  return (
    <pre className="whitespace-pre-wrap text-[1.075rem] leading-relaxed text-slate-100">
      {out}
      {!done && <span className="ml-0.5 text-amber-300 animate-pulse">▍</span>}
    </pre>
  );
}
