...
/* ─────────────── Content (Scene 2 + SUPER CHUNK) ─────────────── */
export const Act1_Scene2: Mission = {
  id: "act-1-hotel-scene2",
      ...
      blocks: [
        {
          ...
          options: [
            ...
            },
            {
              ...
              correct: false,
              xp: -3,
              feedback: "The receptionist narrows her eyes and turns back toward the TV. Try again.",
      ...
      blocks: [
        {
          ...
          options1: [
            ...
            },
            {
              ...
              correct: false,
              xp: -3,
              feedback: "Nope. Try again. 🕵️",
              feedbackTr: "Nop. Inténtalo de nuevo. 🕵️"
            }
          ],
          // colored last fragment
          prompt2: `Translate into Spanish: <br/><span style="color:#fbbf24">What do you mean it’s full?</span>`,
...
export default function Page() {
  return (
    <div className="relative">
      <MissionRunner mission={Act1_Scene2} />

      {/* Sticky NAV buttons */}
      <div className="fixed right-4 bottom-4 z-[1100] flex gap-2">
        <Link
          href="/missions/inheritance/act-1/scene1"
...
export function MissionRunner({ mission }: { mission: Mission }) {
  ...
  // Scene-driven hero image (same rules as original)
  const heroImage =
    currentSceneIdx >= 4
      ? "/images/booking.png"
      : currentSceneIdx >= 3
        ...
        <div className="rounded-3xl bg-[#0b1518]/80 backdrop-blur-xl border border-slate-800/80 shadow-[0_25px_60px_-25px_rgba(0,0,0,0.8)] p-6 sm:p-8">
          ...
          {current?.kind === "superChunk" && (
            <SuperChunkBlockView
              ...
              confetti={confetti}
              onAward={(d) => awardXP(d)}
              fireXp={(amt, x, y) => fireXp(amt, x, y)}   // ← added toasts
              onNext={() => next()}
              onBack={back}
            />
          )}
...
function ChoiceWithLeadBlock({
  ...
  // 1) While the lead is typing, show only the typewriter + a disabled NavRow
  if (!leadDone) {
    return (
      <div className="space-y-5">
        <TypeLine text={block.lead} onDone={handleLeadDone} audio={audio} />
        <NavRow onBack={onBack} showNext={false} />
      </div>
    );
  }

  // 2) After lead finishes, show the prompt + options + feedback + Nav
  return (
    <div className="space-y-5">
      <TypeLine text={block.lead} onDone={() => { /* no-op after first run */ }} audio={audio} />
...
function SuperChunkBlockView({
  ...
  fireXp: (amount: number, x: number, y: number) => void; // ← NEW
  onNext: () => void;
  onBack: () => void;
}) {
  ...
  function handleMcq(e: React.MouseEvent<HTMLButtonElement>, o: Option) {
    ...
    setMcqPicked(o.id);
    onAward(o.xp);
    fireXp(o.xp, e.clientX, e.clientY);              // ← toast for MCQ
    if (o.correct) {
      sfx.correct();
  ...
  function stripDiacritics(s: string) {
    ...
    try { return n.replace(/\p{Diacritic}/gu, ""); } catch { return n.replace(/[\u0300-\u036f]/g, ""); }
  }
  ...
  function gradeTranslation(e: React.MouseEvent<HTMLButtonElement>) {
    if (result2 || !hasTranslation) return;
    const gold = normalize(block.answer2!);
    const user = normalize(input);

    let xp = block.xpWrong ?? -3;
    let ok = false;
    let msg = `Not quite. Aim for: <span class='text-green-600 font-bold'>¿Cómo que está lleno?</span>`;

    if (user === gold) {
      xp = block.xpExact ?? 20; ok = true; msg = "Perfecto — nailed the nuance. 🏅";
    } else if (user.startsWith("como que") && (user.includes("esta lleno") || user.includes("esta llena"))) {
      xp = block.xpFuzzy ?? 5; ok = true; msg = "Nice! That’s a valid variation — you got the structure.";
    }

    onAward(xp);
    fireXp(xp, e.clientX, e.clientY);                // ← toast for translation
  ...
  // Build the “action button” for the translation area without a ternary-in-JSX
  let translationAction: React.ReactNode = null;
  if (hasTranslation) {
    if (!result2) {
      translationAction = (
        <button
          ...
          className={`min-w-36 text-center text-lg px-6 py-3 rounded-2xl font-extrabold text-slate-900 transition
            ${(!mcqDone || input.trim().length === 0)
              ...
              : "bg-emerald-300 hover:bg-emerald-200 cursor-pointer"} shadow-[0_10px_30px_-10px_rgba(110,231,183,0.6)]`}
        >
          Check answer
        </button>
      );
      ...
      {hasTranslation && (
        <div className="rounded-2xl border border-emerald-700/40 bg-[#0f1f23]/70 p-5 shadow-[inset_0_0_0_1px_rgba(16,185,129,0.2)]">
          ...
          <div className="mt-4 flex items-center justify-between">
            <button
              onClick={onBack}
              className="min-w-32 text-center text-base px-4 py-2 rounded-xl fontWeight: 600 text-slate-100 bg-slate-800/70 hover:bg-slate-700/70 border border-slate-600/60 shadow transition cursor-default hover:cursor-pointer"
            >
              ← Go back
            </button>
...
{result2 && (
  ...
  >
    <span dangerouslySetInnerHTML={{ __html: result2.msg }} />
      ...
      {/* Continue button for MCQ-only case */}
      {!hasTranslation && (
        <Link
          href="/missions/inheritance/act-1/scene3"
          className={`min-w-36 text-center text-lg px-6 py-3 rounded-2xl fontWeight: 600 text-slate-900 transition
            ${!mcqDone ? "bg-slate-600/60 cursor-not-allowed" : "bg-emerald-300 hover:bg-emerald-200 cursor-pointer"} shadow-[0_10px_30px_-10px_rgba(110,231,183,0.6)]`}
        >
          Continue
        </Link>
      )}
          ...
          <div className="space-y-2">
            ...
            {left.map((p) => {
              ...
              return (
                ...
                >
                  <span className="fontWeight: 600 text-slate-100">{p.es}</span>
                </button>
              );
            })}
          </div>
