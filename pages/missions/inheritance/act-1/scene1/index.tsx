...
/* ─────────────── Content (Scene 1 only) ─────────────── */
export const Act1_Scene1: Mission = {
  id: "act-1-hotel-scene1",
  ...
  preload: [
    {
      ...
      text:
        ...
        "Half-asleep, you ramble in English to the taxi driver, your eyes drifting between neon signs and sprawling avenues.\n" +
        "After what feels like an eternity, the cab slows to a stop in front of Hotel San Lázaro.\n" +
        "You thank the driver, haul your bag onto the sidewalk, and turn toward the entrance.\n" +
        "The heavy glass doors swing open. A wave of cool air hits you.\n" +
        "You’ve been awake for hours, but there’s no time to collapse.\n" +
        "First, you need to check in before your brain shuts down."
    },
    {
      kind: "card",
  ...
  scenes: [
    {
      id: "scene-1",
      title: "SCENE 1 – The Cold Shoulder", // ← removed 🎭 emoji
      blocks: [
        {
          kind: "choiceLead",
          lead:
            "You walk through the glass doors and walk gingerly to reception.\n" +
            "The receptionist doesn’t even bother to turn to look at you.\n" +
          ...
          options: [
            ...
            },
            {
              ...
              translation: "Good afternoon. I has a reservation for tonight.",
              correct: false,
              xp: -3,
              feedback:
                // Added guidance BEFORE the original line, with color highlights
                "Careful! It’s <span class='text-green-600 font-bold'>tengo</span>, not <span class='text-red-600 font-bold'>tiene</span>. " +
                "You can also skip the unnecessary <span class='text-red-600 font-bold'>yo</span> 😊 <br/><br/>" +
                "The receptionist stares at you blankly. On the TV, someone else gets slapped. Coincidence? Probably not. Try again.",
              feedbackTr:
                "¡Ojo! Es <span class='text-green-600 font-bold'>tengo</span>, no <span class='text-red-600 font-bold'>tiene</span>. " +
                "Además, usa <span class='text-green-600 font-bold'>para</span> (propósito) y omite el <span class='text-red-600 font-bold'>yo</span> innecesario. 😊 " +
...
export default function Page() {
  return (
    <div className="relative">
      <MissionRunner mission={Act1_Scene1} />
...
export function MissionRunner({ mission }: { mission: Mission }) {
  ...
  const goNext = React.useCallback(() => {
    if (isFinalStep) router.push("/missions/inheritance/act-1/scene2");
    else next();
        ...
        <div className="rounded-3xl bg-[#0b1518]/80 backdrop-blur-xl border border-slate-800/80 shadow-[0_25px_60px_-25px_rgba(0,0,0,0.8)] p-6 sm:p-8">
          ...
          {current?.kind === "dialogueWithChoice" && (
            <DialogueWithChoiceBlock
              ...
              blip={blip}
              audio={audio}
              onNext={goNext}   // advance AFTER correct answer
              onBack={back}
              nextLabel={nextLabel}
            />
          )}
      ...
      <div className={`relative transition-opacity ${delayReady ? "opacity-100" : "opacity-70"}`}>
        ...
        >
          ...
          <div className="mt-3 text-[11px] text-emerald-200/70 select-none hover:cursor-pointer">
            {!delayReady
              ? "Please wait…"
              : (doneTyping ? (atLastLine ? `Click ${nextLabel} below.` : "Click to continue →") : "Click to skip typing")}
          </div>
        </div>
      </div>
